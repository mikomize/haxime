...
class Haxime:
    ...
    def make_snippet(self, tmp):
        ...
        if len(m) > 0 and re.match(r"^this", m[0]) is not None:  # yeah i suck at regexp, too lazy merge to it with the one above
            m.pop(0)
        toJoin = []
        i = 0
...
class HaximeWatcher(sublime_plugin.EventListener):
    ...
    def on_modified(self, view):
        if view.file_name() is None or not haxime.plugin_enabled(view):
            return
...
class HaximeBuild(sublime_plugin.WindowCommand):
    def run(self):
        ...
        if haxime.plugin_enabled(view) is False or haxime.build_system_enabled(view) is False:
